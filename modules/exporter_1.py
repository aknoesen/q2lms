...
class CanvasLaTeXQTIGenerator:
    ...
    # Updated create_qti_package function for exporter.py

    def create_qti_package(df, original_questions, quiz_title=None, transform_json_to_csv=None, csv_to_qti=None):
        """Canvas-Compatible QTI Package Creation with Enhanced UI Controls"""
        ...
        st.subheader("📦 QTI Package Configuration")
        
        col1, col2 = st.columns([2, 1])
        
        with col1:
            # Quiz title input
            default_title = quiz_title if quiz_title else "Canvas_Quiz"
            quiz_name = st.text_input(
                "Quiz Title", 
                value=default_title,
                help="This will be the quiz name in Canvas"
            )
        ...
        with col3:
            ...
            safe_quiz_name = sanitize_filename(quiz_name) if quiz_name else "Canvas_Quiz"
            default_filename = f"{safe_quiz_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            
            custom_filename = st.text_input(
                "QTI Package Filename (without .zip)", 
                value=default_filename,
                help="Specify the filename for your QTI package. Will be saved to Downloads folder."
            )
        ...
        if custom_filename:
            ...
            if safe_filename != custom_filename:
                st.warning(f"⚠️ Filename adjusted for compatibility: `{safe_filename}.zip`")
        ...
        with st.expander("📋 Export Preview", expanded=False):
            st.write(f"**Quiz Title in Canvas:** {quiz_name}")
            st.write(f"**Questions to Export:** {len(df)}")
            st.write(f"**Filename:** `{final_filename}.zip`")
            st.write(f"**Download Location:** `C:\\Users\\{os.environ.get('USERNAME', 'User')}\\Downloads\\{final_filename}.zip`")
        
        # Generate QTI Package Button
        if st.button("🚀 Create QTI Package", type="primary"):
            try:
                with st.spinner("🔄 Creating Canvas-compatible QTI package..."):
                    ...
                    df = fix_dataframe_dtypes(df)
                    if show_debug:
                        st.info("✅ DataFrame data types fixed")
                    
                    # Filter and sync questions
                    if show_debug:
                        st.info(f"📋 Processing {len(df)} questions with Canvas LaTeX conversion...")
                    
                    filtered_questions = filter_and_sync_questions(df, original_questions, show_debug)
                    ...
                    if show_debug:
                        st.success(f"✅ {len(filtered_questions)} questions processed successfully")
                    
                    # Fix numeric formatting in the filtered questions
                    filtered_questions = fix_numeric_formatting(filtered_questions)
                    if show_debug:
                        st.info("✅ Numeric formatting standardized")
                    
                    # Generate QTI package
                    qti_generator = CanvasLaTeXQTIGenerator()
                    qti_package_data = qti_generator.create_qti_package(filtered_questions, quiz_name)
                    
                    if qti_package_data:
                        if show_debug:
                            st.success(f"✅ QTI XML generated ({len(qti_package_data):,} bytes)")
                        
                        # Create download
                        st.success("✅ Canvas-compatible QTI package created successfully!")
                        
                        # Enhanced download button with custom filename
                        st.download_button(
                            label=f"📦 Download {final_filename}.zip",
                            data=qti_package_data,
                            file_name=f"{final_filename}.zip",
                            mime="application/zip",
                            key=f"qti_download_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                            help=f"Download QTI package as {final_filename}.zip"
                        )
                        ...
                        **Ready for Canvas Import:** Go to Canvas → Quizzes → Import QTI Package
                        ...
                        with st.expander("📊 Technical Details", expanded=False):
                            st.write("**LaTeX Conversion Applied:**")
                            st.write("- Inline math converted to Canvas format (`$...$` → `\\(...\\)`)")
                            st.write("- Block math preserved (`$$...$$`)")
                            st.write("- Greek letters, subscripts, units preserved exactly")
                            st.write("- HTML entities properly escaped for Canvas")
                            
                            if show_debug:
                                st.write("**Debug Information:**")
                                st.write(f"- Package size: {len(qti_package_data):,} bytes")
                                st.write(f"- Questions processed: {len(df)} → {len(filtered_questions)}")
                                st.write(f"- LaTeX expressions found: {latex_count}")
                    else:
                        st.error("❌ Failed to create QTI package")
    ...
    # Updated filter_and_sync_questions function to handle debug mode
    def filter_and_sync_questions(df, original_questions, show_debug=False):
        """Filter and sync questions while preserving LaTeX - with optional debug output"""
    ...
    # Additional helper function for better filename handling
    import os
    ...
    def sanitize_filename(filename):
        """Create safe filename with enhanced validation"""
        if not filename:
            return "Question_Package"
